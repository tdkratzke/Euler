...
public class Euler0122 {
	...
	public static void main(final String[] args) {
		...
		NMULTS_LOOP : for (int nMults = 1; nMults < maxPower; ++nMults) {
			...
			/**
			 * For each set thisSet, compute the new powers that would result from an additional
			 * multiplication. For each new power newPower, add newPower to set to form newSet
			 * and store newSet in newSets.
			 */
						...
						if (!completed.get(newPower)) {
							...
							if (nMults > _BigNMults) {
								if (nToGo > _SmallNUNknowns) {
									System.out.printf("\nm(%03d) = %d, %d new sets, %d unknown m-values.",
											newPower, nMults, newSets.size(), nToGo);
								} else {
									System.out.printf("\nm(%03d) = %d, %d new sets, Unknowns: ", newPower,
											nMults, newSets.size());
								}
							}
